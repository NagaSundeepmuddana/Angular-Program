...
export class TripComponent implements OnInit {
  ...
  constructor(@Inject(Http) http: Http,private router: Router,private route: ActivatedRoute, private metaService: MetadataService, private userService: UserService, public auth: AuthenticationService, private projectService: ProjectService, ) 
  {
  ...
  update_trip() {
    console.log('sundeep..',this.custid);
    
    this.custid    
    this.userProfile = JSON.parse(localStorage.getItem('userprofile'));
    var TripReport = {
      CustomerID:parseInt(this.custid),
      TravelledDate: $('#ToDate input').val(),
      PeopleTavelled: $('#PeopleTavelled textarea').val(),
      Itinerary: $('#Itinerary textarea').val(),
      CustomerInformation: $('#CustomerInformation textarea').val(),
      SummaryOfMeeting: $('#SummaryOfMeeting textarea').val(),
      ActionItems: $('#ActionItems textarea').val(),
      AdditionalFollowup: $('#AdditionalFollowup textarea').val(),      
      CreatedBy: this.userProfile.sub

    }
    console.log($('#PeopleTavelled input').val());
    this.projectService.AddTripreport(TripReport).subscribe(data => {
